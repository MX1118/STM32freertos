...
static const uint8_t OLED_InitCmds_SSD1306[] = {
	0xAE,
	0xD5, 0x80,
	0xA8, 0x3F,
	0xD3, 0x00,
...
static const uint8_t OLED_InitCmds_SH1106[] = {
	0xAE,
	0xD5, 0x80,
	0xA8, 0x3F,
	0xD3, 0x00,
...
static const OLED_Config OLED_ConfigTable[] = {
	{OLED_CTRL_SSD1306, 0, OLED_InitCmds_SSD1306, sizeof(OLED_InitCmds_SSD1306)},
	{OLED_CTRL_SH1106, 2, OLED_InitCmds_SH1106, sizeof(OLED_InitCmds_SH1106)},
};

static const OLED_Config *OLED_CurrentConfig = &OLED_ConfigTable[0];
...
static void OLED_SelectConfig(void)
{
	uint8_t i;
	OLED_CurrentConfig = &OLED_ConfigTable[0];
	for (i = 0; i < sizeof(OLED_ConfigTable) / sizeof(OLED_ConfigTable[0]); i++)
	{
		if (OLED_ConfigTable[i].Controller == OLED_DEFAULT_CTRL)
		{
			OLED_CurrentConfig = &OLED_ConfigTable[i];
			break;
		}
	}
}
...
static void OLED_I2C_SkipAck(void)
{
	OLED_W_SDA(1);
	OLED_W_SCL(1);
	OLED_W_SCL(0);
...
{
	OLED_I2C_SendByteRaw(Byte);
#if OLED_ENABLE_ACK_CHECK
...
static uint8_t OLED_WriteCommand(uint8_t Command)
{
	uint8_t Success;
	OLED_I2C_Start();
	Success = OLED_I2C_SendByte((uint8_t)(OLED_I2C_ADDRESS << 1));
	Success &= OLED_I2C_SendByte(0x00);
	Success &= OLED_I2C_SendByte(Command);
	OLED_I2C_Stop();
	return Success;
}

static uint8_t OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	uint8_t i;
	uint8_t Success;
	OLED_I2C_Start();
	Success = OLED_I2C_SendByte((uint8_t)(OLED_I2C_ADDRESS << 1));
	Success &= OLED_I2C_SendByte(0x40);
	for (i = 0; i < Count; i ++)
	{
		OLED_I2C_SendByteRaw(Data[i]);
		OLED_I2C_SkipAck();
	}
	OLED_I2C_Stop();
	return Success;
...
static uint8_t OLED_SendInitSequence(void)
{
	uint8_t i;
	uint8_t Success = 1;
	for (i = 0; i < OLED_CurrentConfig->InitCmdCount; i++)
	{
		Success &= OLED_WriteCommand(OLED_CurrentConfig->InitCmds[i]);
		if (!Success)
		{
			break;
		}
...
void OLED_Init(void)
{
	...
	for (Retry = 0; Retry < OLED_INIT_RETRY_COUNT; Retry++)
	{
		OLED_I2C_BusRecover();
...
{
	X += OLED_CurrentConfig->ColumnOffset;
